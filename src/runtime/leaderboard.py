                    ...
                    if userdata: 
                        ...
                        if ctf_state:
                            ...
                            if int(user_total_score) > -1:
                                ...
                                user_teams = user_teams if len(user_teams) > 0 else guardian_state.get("teams.history")    
...
def update_leaderboard_webpage(leaderboard : list) -> None:
    leaderboard_json = os.path.join(os.getcwd(), "..", "leaderboard_web", "leaderboard.txt")
    ...
    for placing_array in leaderboard:
        ...
        for user in top_users:
            if idx < MAX_LEADERBOARD_VIEW:
                ...
                file_lines.append(
                    f"\"{idx}\"" + " : {" + f""""username" : "{user["name"]}","score" : {total_score}""" + "},\n"
                )
                idx += 1
