...
class AdminConsole(object):
    ...
    def entry_admin(self, update: Update, context: CallbackContext) -> USERSTATE:
        ...
        if user.chatid in ADMIN_USERS:
            user.logger.info(True, "USER_IS_ADMIN_USER", f"User:{user.chatid} is an  admin, loading admin console")
            return self.load_admin(update, context)
        else:
            user.logger.info(True, "USER_IS_NORMAL_USER", f"User:{user.chatid} does not have admin privilege, skipping admin console")
    ...
    def setup(self, stage_id : str, next_stage_id : str) -> None:
        ...
        self.stage = self.bot.add_stage(
            ...
            states={
                "MENU" : [
                    CallbackQueryHandler(self.prompt_delete_user_name, pattern="^admin_delete_user_name$", run_async=True),
                    CallbackQueryHandler(self.prompt_delete_user, pattern="^admin_delete_user$", run_async=True),
                    CallbackQueryHandler(self.prompt_ban_user, pattern="^admin_ban_user$", run_async=True),
                    CallbackQueryHandler(self.prompt_unban_user, pattern="^admin_unban_user$", run_async=True),
                    CallbackQueryHandler(self.exit_admin, pattern="^admin_exit$", run_async=True),
                ]
            }
        )
    ...
    def load_admin(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        self.bot.edit_or_reply_message(
            ...
            reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("Delete User's Name", callback_data="admin_delete_user_name")],
                    [InlineKeyboardButton("Delete User", callback_data="admin_delete_user")],
                    [InlineKeyboardButton("Ban User", callback_data="admin_ban_user")],
                    [InlineKeyboardButton("Unban User", callback_data="admin_unban_user")],
                    [InlineKeyboardButton("Back to Bot", callback_data="admin_exit")]
            ])
        )
