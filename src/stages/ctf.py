...
class Ctf(object):
    ...
    def load_challenges(self) -> None:
        ...
        for challenge_idx, _ in enumerate(os.listdir(self.challenges_directory)):
            ...
            if os.path.isfile(challenge_yaml_file):
                challenge_data = utils.load_yaml_file(challenge_yaml_file, self.bot.logger)
    ...
    def setup(self, stage_id : str, next_stage_id : str = "end") -> None:
        ...
        menu_view_callbacks = [
            CallbackQueryHandler(self.view_leaderboard, pattern="^ctf_view_leaderboard$", run_async=True),
            CallbackQueryHandler(self.exit_ctf, pattern="^ctf_exit$", run_async=True)
        ]
        challenge_view_callbacks = [CallbackQueryHandler(self.load_menu, pattern="^ctf_return_to_menu$", run_async=True)]
        retry_challenge_callbacks = [CallbackQueryHandler(self.load_menu, pattern="^ctf_return_to_menu$", run_async=True)]
        
        for idx_c, challenge in enumerate(self.challenges):
            menu_view_callbacks.append(
                CallbackQueryHandler(self.view_challenge, pattern=f"^ctf_menu_view_challenge_{idx_c}$", run_async=True)
            )
            challenge_view_callbacks.extend([
                CallbackQueryHandler(self.submit_answer, pattern=f"^ctf_submit_answer_{idx_c}$", run_async=True),
                CallbackQueryHandler(self.view_challenge, pattern=f"^ctf_refresh_challenge_{idx_c}$", run_async=True)
            ])
            retry_challenge_callbacks.extend([
                CallbackQueryHandler(self.submit_answer, pattern=f"^ctf_submit_answer_{idx_c}$", run_async=True),
                CallbackQueryHandler(self.view_challenge, pattern=f"^ctf_return_to_challenge_{idx_c}$", run_async=True)
            ])
            
            if challenge["multiple_choices"]:
                for idx_a, _ in enumerate(challenge["multiple_choices"]):
                    challenge_view_callbacks.append(
                        CallbackQueryHandler(self.submit_choice_answer, pattern=f"^ctf_select_choice_{idx_a}:{idx_c}$", run_async=True)
                    )
            
            for idx_h, _ in enumerate(challenge["hints"]):
                challenge_view_callbacks.append(
                    CallbackQueryHandler(self.reveal_hint, pattern=f"^ctf_view_hint_{idx_h}:{idx_c}$", run_async=True)
                )
    ...
    def load_menu(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        user.logger.info(True, "USER_CTF_LOAD_MENU", f"User:{user.chatid} has loaded ctf menu")
        ...
        for idx, challenge in enumerate(ctf_state["challenges"]):
            ...
            challenges_to_attempt = True if (not is_challenge_completed and not challenge["one_try"]) else challenges_to_attempt
        ...
        if self.leaderboard_active:
            keyboard.append([InlineKeyboardButton("Leaderboard 📈", callback_data="ctf_view_leaderboard")]) 
        keyboard.append([InlineKeyboardButton("Exit 👋", callback_data="ctf_exit")]) 
        ...
        score_type_msg = "final" if (all_challenges_completed or not challenges_to_attempt) else "current"
    ...
    def view_challenge(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        user : User = context.user_data.get("user")
        user.logger.info(True, f"USER_CTF_VIEW_CHALLENGE_{challenge_number}", f"User:{user.chatid} has viewed Challenge {challenge_number}")
        ctf_state = user.data.get("ctf_state")
        ...
        if challenge["time_based"] and not challenge["time_based"]["start_time"]:
            ...
            for i in range(delay_before_revealing):
                self.bot.edit_or_reply_message(
                    ...
                    + MESSAGE_DIVIDER + f"Revealing challenge in: <b>{delay_before_revealing-i}</b>",
                )
                time.sleep(1)
            
            # Updating and saving players data
            challenge["time_based"].update({"start_time" : datetime.datetime.now()})
    ...
    def reveal_hint(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        user : User = context.user_data.get("user")
        user.logger.info(True, f"USER_CTF_VIEW_HINT_{challenge_number}_{hint_number}", f"User:{user.chatid} has revealed hint {hint_number} for Challenge {challenge_number}")
        ctf_state = user.data.get("ctf_state")
        ...
        challenge.update({"total_hints_deduction" : challenge["total_hints_deduction"] + hint["deduction"]})
        user.save_user_to_file()
    ...
    def submit_choice_answer(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        challenge_number, choice_number = int(challenge_number), int(choice_number)
        
        user : User = context.user_data.get("user")
        user.logger.info(True, f"USER_CTF_SUBMIT_{challenge_number}", f"User:{user.chatid} is submitting choiced answer {choice_number} for Challenge {challenge_number}")
        ctf_state = user.data.get("ctf_state")
    ...
    def submit_answer(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        user : User = context.user_data.get("user")
        user.logger.info(True, f"USER_CTF_SUBMIT_{challenge_number}", f"User:{user.chatid} is submitting answer for Challenge {challenge_number}")
        ...
        context.user_data.update({"ctf_message_handler_debounce" : challenge_number})
        return self.SUBMIT_CHALLENGE
    
    
    def handle_answer(self, update : Update, context : CallbackContext) -> USERSTATE:
        challenge_number = context.user_data.get("ctf_message_handler_debounce")
    ...
    def view_leaderboard(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        user : User = context.user_data.get("user")
        user.logger.info(True, f"USER_CTF_VIEW_LEADERBOARd", f"User:{user.chatid} is viewing the leaderboard.")
        ctf_state = user.data.get("ctf_state")
    ...
    def display_challenge(self, update : Update, context : CallbackContext, challenge_number : int) -> None:
        ...
        can_attempt = (challenge["one_try"] and challenge["attempts"] == 0) or (not challenge["one_try"])
        ...
        if not is_challenge_completed:
            ...
            if can_attempt and not is_multiple_choices:
                keyboard.append([InlineKeyboardButton("Submit answer", callback_data=f"ctf_submit_answer_{challenge_number}")])
            ...
            if is_multiple_choices:
                ...
                for t_idx, choice in enumerate(challenge["multiple_choices"]):
                    ...
                    keyboard[-1].append(
                        InlineKeyboardButton(f"{choice}", callback_data=f"ctf_select_choice_{t_idx}:{challenge_number}")
                    )
    ...
    def check_answer(self, update : Update, context : CallbackContext, challenge_number : int, answer : str) -> USERSTATE:
        ...
        if answer == answer_key:
            ...
            if challenge["time_based"]:
                ...
                challenge_points = int(challenge_points / challenge["attempts"]) 
            ...
            user.logger.info(True, f"USER_CTF_CORRECT_ANSWER_{challenge_number}", f"""User:{user.chatid} @{ctf_state["total_score"]}@ got the answer CORRECT for Challenge {challenge_number}""")                
            ...
            user.save_user_to_file()
            user.logger.info(True, f"USER_CTF_WRONG_ANSWER_{challenge_number}", f"""User:{user.chatid} @{answer}@ got the answer WRONG for Challenge {challenge_number}""")
