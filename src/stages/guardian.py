...
class Guardian(object):
    ...
    def setup(self, stage_id : str, next_stage_id : str = "end") -> None:
        ...
        self.stage = self.bot.add_stage(
            ...
            states={
                "INTRO_VIEW" : [
                    CallbackQueryHandler(self.load_guardian, pattern="^guardian_begin$", run_async=True),
                    CallbackQueryHandler(self.skip_guardian, pattern="^guardian_skip$", run_async=True)
                ],
                "RESULTS_VIEW" : [
                    CallbackQueryHandler(self.exit_guardian, pattern="^guardian_finished$", run_async=True)
                ]
            }
        )
        self.states = self.stage["states"]
        self.INTRO_VIEW,  self.RESULTS_VIEW = self.bot.unpack_states(self.states)
    ...
    def intro_view(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        keyboard = [InlineKeyboardButton("Begin", callback_data="guardian_begin")]
        
        if len(guardian_state.get("teams", [])) > 0 or len(guardian_state.get("teams.history", [])) > 0:
            if len(guardian_state.get("teams", []))  == 0:
                guardian_state.update({"teams" : list(guardian_state.get("teams.history"))})
                user.save_user_to_file()
            
            keyboard.append(InlineKeyboardButton("Skip", callback_data="guardian_skip"))
    ...
    def calculate_results(self, update : Update, context : CallbackContext) -> None:
        ...
        for team_selected in guardian_state["options_picked"]:
            teams_dict.update({team_selected : teams_dict.get(team_selected, 0) + 1})
        ...
        user.logger.info(True, "USER_IS_TEAM", f"User:{user.chatid} has gotten the results of {teams_picked}")
    ...
    def display_results(self, update : Update, context : CallbackContext) -> USERSTATE:
        ...
        self.bot.edit_or_reply_message(
            ...
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("Next", callback_data="guardian_finished"),
                ]
            ])
        )
