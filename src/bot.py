...
class Bot(object):
    ...
    def proceed_next_stage(self, 
        ...
        next_stage = self.stages.get(next_stage_id) if next_stage_id else self.stages.get(self.states[0]["stage_id"])
        
        if next_stage and user and not user.is_banned:
            user.logger.info(True, "PROCEED_NEXT_STAGE", f"User:{user.chatid} is moving on from: {current_stage_id} to: {next_stage_id}")
            return next_stage["entry"](update, context)
        else:
            if not next_stage:
                user.logger.error(True, "NEXT_STAGE_MISSING", f"Next stage: {next_stage_id} not found. Current stage: {current_stage_id}")
                return self.proceed_next_stage(current_stage_id, "end", update, context)
            elif not user:
                self.logger.error(True, "USER_NOT_REGISTERED", f"User not found in users. Current stage: {current_stage_id}")
            elif user.is_banned:
                self.logger.error(True, "USER_BANNED", f"User not found in users. Current stage: {current_stage_id}")
    ...
    def let_user_choose(self,
        ...
        for idx, choice in enumerate(choices):
            if choices_per_row and idx%choices_per_row == 0:
                keyboard.append([])
            callbacks.append(CallbackQueryHandler(choice["callback"], pattern=f"^{choice_label}:choice:{idx}$", run_async=True))
            keyboard[-1].append(InlineKeyboardButton(choice["text"], callback_data=f"{choice_label}:choice:{idx}"))
    ...
    def get_info_from_user(self, 
        ...
        def prompt_entry(update : Update, context : CallbackContext) -> USERSTATE:
            ...
            if saved_data and saved_data != "" and use_last_saved:
                if allow_update:
                    user.logger.info(True, "USER_DATA_INPUT_UPDATE_PROMPT", f"User:{user.chatid} is choosing whether to update input({data_label})")
                    
                    context.user_data.update({f"input:{data_label}" : saved_data})
                    ...
                    self.edit_or_reply_message(
                        ...
                        reply_markup=InlineKeyboardMarkup([
                            [
                                InlineKeyboardButton("Yes", callback_data=confirm_input_pattern),
                                InlineKeyboardButton("No", callback_data=retry_input_pattern),
                            ]
                        ]),
                    )        
                    ...
                    user.logger.info(True, "USER_DATA_INPUT_UPDATE_FORBIDDEN", f"User:{user.chatid} is forbidden from updating input({data_label}). Using old value...")
                    return prompt_exit(update, context)
            else:
                user.logger.info(True, "USER_DATA_INPUT_INIT_PROMPT", f"User:{user.chatid} is choosing input({data_label})")
                self.edit_or_reply_message(
                    update, context,
        ...
        def input_handler(update : Update, context : CallbackContext) -> USERSTATE:
            ...
            if not debounce and update.message is not None:
                ...
                if formatted_user_input:
                    user.logger.info(True, "USER_DATA_INPUT_CONFIRMATION", f"User:{user.chatid} entered an input({data_label}) of @{user_input}@")
                    
                    context.user_data.update({f"input:{data_label}" : formatted_user_input})
                    ...
                    user.logger.warning(True, "USER_DATA_INPUT_WRONG_FORMAT", f"User:{user.chatid} entered an input({data_label}) of the wrong format. @{user_input}@")
        ...
        def confirm_input(update : Update, context : CallbackContext) -> USERSTATE:
            ...
            user : User = context.user_data.get("user")
            user.logger.info(True, "USER_DATA_INPUT_CONFIRMED", f"User:{user.chatid} confirmed an input({data_label})")
            user.update_user_data(data_label, context.user_data[f"input:{data_label}"])
        ...
        def retry_input(update : Update, context : CallbackContext) -> USERSTATE:
            ...
            user : User = context.user_data.get("user")
            user.logger.info(True, "USER_DATA_INPUT_RETRY", f"User:{user.chatid} retrying an input({data_label})")
        ...
        stage = self.add_stage(
            ...
            states={
                ...
                data_label+"confirmation" : [
                    CallbackQueryHandler(retry_input, pattern=f"^{retry_input_pattern}$", run_async=True),
                    CallbackQueryHandler(confirm_input, pattern=f"^{confirm_input_pattern}$", run_async=True),
                ]
            }
        )
        states = stage["states"]
    ...
    def conversation_entry(self, update : Update, context : CallbackContext) -> USERSTATE:
        if update.message:
            ...
            self.logger.error(True, "USER_MESSAGE_INVALID", f"Unknown user has entered a message with no valid update")
    ...
    def start(self, live_mode : bool = False) -> None:
        ...
        :return: None
        """
        self.add_stage("end", entry=self.conversation_exit, exit=None, states = {})
        
        conversation_states = {}
        for idx, state in enumerate(self.states):
            conversation_states.update({ idx : state["callbacks"] })
        
        start_command = CommandHandler("start", self.conversation_entry, run_async=True)
